...
def build_pose_model(model_cfgs, weights, cuda=True):
...
def build_dataset_with_loader(data_cfgs, tform_cfgs, num_workers=5):
    tform_cfgs.color_jitter = -1  # ignore the color jitter
...
def inference(model, data_set, loader, cuda):
    L = len(data_set)
...
def draw_result(pred_poses, targ_poses, dim3=True, show=False):
    fig = plt.figure()
    if not dim3:
